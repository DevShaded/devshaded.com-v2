...
/**
 * Database seeder for the Prisma database
 *
...
async function main() {
    ...
    const responses = await fetch('https://api.github.com/users/DevShaded/repos');
    const data = await responses.json();

    for (const repo of data) {
        if (repo.owner.login === 'DevShaded') {
            // will return error if records already exists
            // TODO: make sure it doesn't return error if records already exists (props will never happen)
    ...
    await prisma.commands.createMany({
        data:           [
            // Fun commands
            {
                name:        '/8ball',
            ...
            {
                name:        '/unban',
                description: 'Unban a user that has been banned from your server',
                options:     '< target >',
                level:       2,
                permission:  'BAN_MEMBERS',
                category:    'moderation'
